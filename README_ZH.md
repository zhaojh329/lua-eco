...
[7]: https://img.shields.io/badge/发布版本-3.18.0-blue.svg?style=plastic
...
其核心能力是：**基于 I/O 事件自动调度 Lua 协程**（Linux `epoll` + 定时器）。所有 I/O API 底层都使用**非阻塞**文件描述符实现，但在 Lua 层暴露为**同步**写法：当 I/O 暂不可用时，当前协程会自动 `yield`，并在 fd 就绪或超时后自动恢复执行。
...
lua-eco 旨在通过 `eco` 解释器（`#!/usr/bin/env eco`）运行 Lua 程序。
`eco` 模块已内置到该解释器中，因此无法在原生 `lua5.4` 解释器中单独加载。

启动时运行时会自动集成：

- 会自动加载 `eco` 模块并注入到 `_G`。
- 会自动调用 `eco.run(...)` 创建协程来执行用户脚本。
- 会自动调用 `eco.loop()` 启动调度器。

在脚本顶层：

- 不要手动调用 `eco.loop()`。
- 通常不需要手动用 `eco.run()` 包装入口脚本。
- 仍然可以显式写 `local eco = require 'eco'`；它会返回已加载的 `_G.eco`。
    这样可以避免 Lua LSP 的 `Undefined global 'eco'` 告警。
...
local time = require 'eco.time'
local eco = require 'eco' -- 可选：避免 LSP 的 "Undefined global 'eco'"
...
- `shared`: 基于 `mmap(MAP_SHARED)` (`/dev/shm`) 实现的共享内存字典
...
- 直接打开生成的 API 文档：https://zhaojh329.github.io/lua-eco/
