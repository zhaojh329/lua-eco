...
{
    struct eco_socket *sock = lua_newuserdatauv(L, sizeof(struct eco_socket), 0);
...
static int lua_recvfrom(lua_State *L)
{
    ...
    ret = recvfrom(sock->fd, p, n, 0, (struct sockaddr *)&addr, &addrlen);
    if (ret < 0)
        return push_errno(L, errno);
 ...
 * @treturn integer IPv4 address as an integer (network byte order).
 */
 ...
 *
 * @function inet_ntoa
 * @tparam integer addr IPv4 address as an integer (network byte order).
 ...
 *
 * @function inet_ntop
 * @tparam integer family Address family (e.g. `AF_INET`, `AF_INET6`).
 ...
 * @tparam integer Interface index.
