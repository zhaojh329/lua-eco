...
-- @tparam[opt] table params Event payload.
...
-- @tparam[opt] table msg Reply message.
-- @treturn boolean true On success.
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
...
--
-- When the connection is created with `auto_reconnect = true`, event listeners
-- added by this method are automatically re-registered after reconnect.
--
...
-- @tparam function cb Callback `cb(event, msg, con)`.
-- @treturn boolean true On success.
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
function methods:listen(event, cb)
    ...
    self.handlers[obj] = function(con, ev, msg)
        eco.run(cb, ev, msg, con)
    end
...
--
-- When the connection is created with `auto_reconnect = true`, objects added by
-- this method are automatically added again after reconnect.
--
...
-- @tparam function cb Callback `cb(method, msg, con)`.
-- @tparam[opt=false] boolean auto Auto-subscribe when the object appears later.
-- @treturn lightuserdata subscriber Subscriber handle.
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
function methods:subscribe(path, cb, auto)
    if self.closed then return end

    local s, err = self.ctx:subscribe(path, auto)
    if not s then
        return nil, err
    end

    self.handlers[s] = function(con, method, msg)
        eco.run(cb, method, msg, con)
    end

    self.subscribers[s] = true
...
--
-- @function connection:unsubscribe
-- @tparam lightuserdata subscriber Subscriber handle returned by @{connection:subscribe}.
-- @treturn boolean true On success.
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
function methods:unsubscribe(subscriber)
    if self.closed then return end
    ...
    return self.ctx:unsubscribe(subscriber)
end
...
-- @tparam[opt] table params Payload.
-- @treturn boolean true On success.
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
