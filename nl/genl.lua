...
--
-- This module provides convenience helpers for working with Generic Netlink
-- (NETLINK_GENERIC) via @{eco.nl}.
--
...
-- @field id int Numeric family id.
-- @field version int Family version.
-- @field hdrsize int Header size.
-- @field maxattr int Max attribute id.
-- @field groups table Map: group name (string) -> group id (int).
...
--
-- @function get_family_byid
-- @tparam integer id Family id.
-- @treturn family info
...
-- Results are cached in-process.
--
-- @function get_family_byname
-- @tparam string name Family name.
-- @treturn family info
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
...
-- @tparam string family Generic netlink family name.
-- @tparam string group Multicast group name.
-- @treturn int|nil id Group id (or `nil` if not found).
-- @treturn[2] nil On failure.
-- @treturn[2] string Error message.
