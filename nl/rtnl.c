 ...
 */
  ...
  * RTNETLINK helpers.
  *
  * This module provides small helpers to build and parse common rtnetlink
  * message headers used with @{eco.nl}.
  *
  * Typical workflow:
  *
  ...
  *
  * Many constants from Linux headers are also exported (RTM_*, IFF_*, IFLA_*,
  * IFA_*, RTA_*, RTNLGRP_*, RTMGRP_*, RT_SCOPE_*, RTN_*, RTPROT_*).
  *
  * @module eco.rtnl
  */
 ...
 *
 * @function rtgenmsg
 * @tparam table t Table with field:
 *
 * - `family` (int): address family (e.g. `eco.socket.AF_PACKET`).
 *
 * @treturn string Packed `struct rtgenmsg`.
 */
 ...
 * - `change` (int)
 *
 * @treturn string Packed `struct ifinfomsg`.
 */
 ...
 * - `index` (int)
 *
 * @treturn string Packed `struct ifaddrmsg`.
 */
 ...
 * - `flags` (int)
 *
 * @treturn string Packed `struct rtmsg`.
 */
 ...
 *
 * The input message must currently point to an `RTM_NEWLINK` or `RTM_DELLINK`
 * message (see @{eco.nl.nlmsg_ker:next}).
 *
 * @function parse_ifinfomsg
 * @tparam nlmsg_ker msg A kernel message parser from @{eco.nl}.
 * @treturn table Table fields: `family`, `type`, `index`, `flags`, `change`.
 ...
 * @tparam nlmsg_ker msg A kernel message parser from @{eco.nl}.
 * @treturn table Table fields: `family`, `prefixlen`, `flags`, `scope`, `index`.
 * @treturn[2] nil On failure.
 * @treturn[2] string Error message.
 */
 ...
 *
 * The input message must currently point to an `RTM_NEWROUTE` or `RTM_DELROUTE`
 * message.
 *
 * @function parse_rtmsg
 * @tparam nlmsg_ker msg A kernel message parser from @{eco.nl}.
 * @treturn table Table fields: `family`, `dst_len`, `src_len`, `tos`, `table`,
